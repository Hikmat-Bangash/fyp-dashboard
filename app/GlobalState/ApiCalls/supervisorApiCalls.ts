...
export const GetSingleSupervisor = async (
  ...
  try {
    const res = await API.get(`api/v1/supervisors/${supervisorId}`, ConfigCredential);
...
export const DeleteSupervisor = async (
  ...
  dispatch(ApiRequestStart());
  try {
    const res = await API.delete(`api/v1/supervisors/${supervisorId}`, ConfigCredential);
...
export const FetchAllSupervisors = async (dispatch: any): Promise<any> => {
  dispatch(ApiRequestStart());
  try {
    const res = await API.get("api/v1/supervisors", ConfigCredential);
